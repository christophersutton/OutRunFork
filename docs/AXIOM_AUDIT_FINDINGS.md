...
- Edit-workout snapshot red: `xcodebuild test -workspace OutRun.xcworkspace -scheme OutRun -destination 'id=D8A7B779-0769-41A2-B000-760C64691084' -only-testing:UnitTests/CoreStorePresentationSnapshotTests -resultBundlePath /tmp/OutRun-CoreStorePresentationSnapshot-green.xcresult` built and failed the new source-shape assertions before the matcher was narrowed, proving the guard would reject public-accessor-shaped edit seeding. Earlier pre-pod-install attempts failed before tests due missing CocoaPods modules at `/tmp/OutRun-CoreStorePresentationSnapshot-red2.xcresult`.
- Edit-workout snapshot green: `xcodebuild test -workspace OutRun.xcworkspace -scheme OutRun -destination 'id=D8A7B779-0769-41A2-B000-760C64691084' -only-testing:UnitTests/CoreStorePresentationSnapshotTests -resultBundlePath /tmp/OutRun-CoreStorePresentationSnapshot-green2.xcresult` passed with 2 selected tests and 0 failures after `WorkoutEditSnapshot`, the async query API, and the edit-form loading path were added.
- Edit-workout adjacent regression: `xcodebuild test -workspace OutRun.xcworkspace -scheme OutRun -destination 'id=D8A7B779-0769-41A2-B000-760C64691084' -only-testing:UnitTests/CoreStoreTempConversionThreadingTests -resultBundlePath /tmp/OutRun-CoreStoreTempConversionThreading-regression.xcresult` passed with 2 selected tests and 0 failures.
...
- `OutRun/Models/Data/Snapshots/WorkoutEditSnapshot.swift` seeds the edit form from raw `_workoutType.value`, `_distance.value`, `_steps.value`, `_startDate.value`, `_endDate.value`, `_isRace.value`, and `_comment.value` fields.
- `DataManager.workoutEditSnapshot(for:)` builds that snapshot inside `dataStack.perform` with `queryObject(from:transaction:)`.
- `EditWorkoutForm` now keeps edit mode behind a loading placeholder until the async snapshot is applied; create mode still initializes synchronously with defaults, and save still re-resolves the live workout for update semantics.
- `UnitTests/CoreStorePresentationSnapshotTests.swift` guards the edit-workout presentation seeding path by rejecting `DataManager.queryObject` in `EditWorkoutState.init(mode:)`, rejecting `workout.<public accessor>` reads in edit seeding/snapshot creation, and requiring the async snapshot query API shape.
...
Status: Partially addressed 2026-06-21. Added explicit localized `LS["Close"]` accessibility labels to the icon-only dismiss buttons in workout detail, debug, policy, and changelog SwiftUI screens, and added `UnitTests/CloseButtonAccessibilityTests.swift` to keep those labels on the `Button` views. The Dynamic Type source-shape slice now covers all app Swift sources for fixed SwiftUI/UIKit text-font APIs, converts the reviewed timeline stats and UIKit label/button examples to semantic/preferred fonts, and keeps the fixed-font allowlist narrowed to decorative brand text and symbol-only icon sizing.
...
- Adversarial Dynamic Type red: `xcodebuild test -workspace OutRun.xcworkspace -scheme OutRun -destination 'id=D8A7B779-0769-41A2-B000-760C64691084' -only-testing:UnitTests/DynamicTypeFontShapeTests -resultBundlePath /tmp/OutRun-DynamicType-red-test.xcresult` failed with expected violations for timeline `WorkoutStatText.bigStat`, shorthand UIKit `.systemFont(ofSize:)` labels/buttons, and `HKImportListController`.
...
- Adversarial Dynamic Type green: `xcodebuild test -workspace OutRun.xcworkspace -scheme OutRun -destination 'id=D8A7B779-0769-41A2-B000-760C64691084' -only-testing:UnitTests/DynamicTypeFontShapeTests -resultBundlePath /tmp/OutRun-DynamicType-green.xcresult` passed with 1 selected test and 0 failures.
- Regression: `xcodebuild test -workspace OutRun.xcworkspace -scheme OutRun -destination 'platform=iOS Simulator,id=D6C217FB-9E68-4CA2-B9FC-A5202C44A667' -derivedDataPath /tmp/outrun-audit-slices-green19 -resultBundlePath /tmp/outrun-audit-slices-green19-dynamic-type-final.xcresult -only-testing:UnitTests` passed with 47 tests and 0 failures.

The SwiftUI replacement surfaces and legacy UIKit views had many fixed font sizes, risking clipping, poor Large Content Size behavior, and inconsistent VoiceOver output. This slice addresses the static fixed text-font matches covered by the source-shape test, but did not perform visual/manual VoiceOver validation or runtime content-size-change testing.

Evidence:
- Fixed: audited SwiftUI text fonts in onboarding, timeline, workout detail, policy/changelog, and shell surfaces now use semantic fonts such as `.body`, `.title2`, `.largeTitle`, `.subheadline`, `.caption`, and `.caption2`.
- Fixed: `OutRun/Views/SwiftUI/Timeline/WorkoutTimelineRow.swift` now renders primary stats with semantic `WorkoutStatText.bigStat(..., style: .largeTitle/.title2)` instead of fixed 36/24-point system fonts.
...
- Fixed: `OutRun/Controllers/HealthKit/HKImportListController.swift`, `OutRun/Views/HealthKit/HKImportObjectCell.swift`, `OutRun/Views/Workout/WorkoutCompletionBanner.swift`, `OutRun/Views/Data/LabelledDataView.swift`, `OutRun/Models/ORBanner/TextBanner.swift`, `OutRun/Views/Data/LoadingView.swift`, `OutRun/Views/General/FloatingButton.swift`, `OutRun/Views/Workout/WorkoutBuilderReadinessIndicationView.swift`, and `OutRun/Views/Workout/WorkoutBuilderTypeView.swift` now use preferred Dynamic Type text styles for user-facing UIKit text.
- Fixed: `OutRun/Extensions/UIKit/UILabel.swift` enables `adjustsFontForContentSizeCategory` when the convenience initializer receives a font, and manually configured button title labels in the touched UIKit files enable the same adjustment.
- Fixed: `OutRun/Extensions/UIKit/UISegmentedControl.swift` now uses a preferred caption font for segmented title attributes and registers a content-size-category observer that reapplies those attributes while the styled control is alive.
- Guarded: `UnitTests/DynamicTypeFontShapeTests.swift` scans all app Swift sources for fixed text font APIs while explicitly allowlisting intentional brand display text and symbol-only icon sizing.
